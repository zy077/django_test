var vm = new Vue({
    ...
    created: function(){
        ...
        axios.get(this.host + '/user/'+user_id+'/addresses/', {
                headers: {
    ...
    methods: {
        ...
        save_address: function(){
            if (this.error_receiver || this.error_place || this.error_mobile || this.error_email || !this.form_address.province_id || !this.form_address.city_id || !this.form_address.district_id ) {
                ...
                if (typeof(this.editing_address_index) == "number") {
                    // 修改地址
                    axios.put(this.host + '/user/' + this.user_id + '/addresses/' + this.addresses[this.editing_address_index].id + '/', this.form_address, {
                        headers: {
                            'Authorization': 'JWT ' + this.token
                        },
                        responseType: 'json'
                    ...
                    // 新增地址
                    axios.post(this.host + '/user/' + this.user_id + '/addresses/', this.form_address, {
                        headers: {
                            'Authorization': 'JWT ' + this.token
                        },
                        responseType: 'json'
        ...
        del_address: function(index){
            axios.delete(this.host + '/user/' + this.user_id + '/addresses/' + this.addresses[index].id + '/', {
                    headers: {
                        'Authorization': 'JWT ' + this.token
                    },
                    responseType: 'json'
        ...
        set_default: function(index){
            axios.put(this.host + '/user/' + this.user_id + '/addresses/' + this.addresses[index].id + '/status/', {}, {
                    headers: {
                        'Authorization': 'JWT ' + this.token
                    },
                    responseType: 'json'
        ...
        save_title: function(index){
            if (!this.input_title) {
                ...
                axios.put(this.host + '/user/' + this.user_id + '/addresses/' + this.addresses[index].id + '/title/', {
